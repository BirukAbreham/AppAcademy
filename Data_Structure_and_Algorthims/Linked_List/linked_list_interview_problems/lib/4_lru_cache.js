...
// Let's code!
// -----------
// Implemented the LRUCacheItem class here
class LRUCacheItem {
...
class LRUCache {
  ...
  // Implemented the size method here
  size() {
    return this.length;
  }

  // Implemented the get method here
  get(key) {
    if (this.map.has(key)) {
  ...
  // Implemented the set method here
  set(key, val) {
    if (!this.map.has(key)) {
      let item = new LRUCacheItem(val, key);
      if (this.isFull()) this.prune();
      let head = this.list.unshift(item);
  ...
  isFull() {
    return this.map.size + 1 > this.limit;
  }
