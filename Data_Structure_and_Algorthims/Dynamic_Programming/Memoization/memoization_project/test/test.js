...
const { coinChange } = require("../lib/leet_code_518");
...
describe("Leet Code #518 - Coin Change 2", () => {
  it("https://leetcode.com/problems/coin-change-2/", () => {
    util.catchStackOverflow(coinChange, [1, 2, 5], 5);

    expect(coinChange([1, 2, 5], 5)).to.equal(4);
    expect(coinChange([2], 3)).to.equal(0);
    expect(coinChange([10], 10)).to.equal(1);
  });
});
