...
function lucasNumberMemo(n, memo = {}) {
  // return if n is in the memo
  if (n in memo) return memo[n];

  if (n === 0) return 2;
  if (n === 1) return 1;

  // if is not in the memo calcualte and append the recursive result
...
function minChange(coins, amount, memo = {}) {
  // return if amount is in the memo
  if (amount in memo) return memo[amount];
  if (amount === 0) return 0;

  for (let i = coins.length - 1; i >= 0; i--) {
    let sub = amount - coins[i];
    if (sub > coins[coins.length - 1] || coins.includes(sub) || sub === 0) {
      memo[sub] = minChange(coins, sub, memo);
      return 1 + memo[sub];
    }
  }
}

function minChangeSln(coins, amount, memo = {}) {
  if (amount === 0) return 0;
  if (amount in memo) return memo[amount];
  ...
  coins.forEach((coin) => {
    if (coin <= amount) {
      num_coins.push(minChangeSln(coins, amount - coin, memo) + 1);
    }
  });
