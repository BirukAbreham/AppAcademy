...
const { canFinish } = require("../lib/leet_code_207");
...
describe("Leet Code #207", () => {
  it("https://leetcode.com/problems/course-schedule/", () => {
    let courseOne = 2;
    let prerequisitesOne = [[1, 0]];
    expect(canFinish(courseOne, prerequisitesOne)).to.equal(true);

    let courseTwo = 2;
    let prerequisitesTwo = [
      [1, 0],
      [0, 1],
    ];
    expect(canFinish(courseTwo, prerequisitesTwo)).to.equal(false);
  });
});
