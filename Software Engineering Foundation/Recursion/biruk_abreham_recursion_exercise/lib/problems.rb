...
def sum_array(array)
  ...
  return sum_array(array[0...array.length-1]) + sum_array([array[-1]])
end

def sum_array_sln(array)
  return 0 if array.empty?

  return array[0] + sum_array_sln(array[1..-1])
end
...
def reverse_string(str)
  ...
  return reverse_string(str[-1]) + reverse_string(str[0...str.length-1])
end

def reverse_string_sln(str)
  return "" if str.empty?
  
  return reverse_string_sln(str[1..-1]) + str[0]
end
...
def flatten(data)
  ...
  if !data.is_a?(Array)
    return [data]
  elsif data.length == 1 && !data[0].is_a?(Array)
    return data
  elsif data.length == 1 && data[0].is_a?(Array)
    return flatten(data[0])
  end
  return flatten(data[0...data.length-1]) + flatten(data[-1])

end

def flatten_sln(data)
  return [data] if !data.is_?(Array)
  
  flattened = []
  data.each do |el|
    flattened += flatten_sln(el)
  end
