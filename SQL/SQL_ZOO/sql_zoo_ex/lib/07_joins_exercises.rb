...
def travoltas_busiest_years
  ...
  execute(<<-SQL)
    ...
    JOIN
      actors ON castings.actor_id = actors.id
    GROUP BY actors.name, movies.yr
    HAVING
      actors.name = 'John Travolta' and COUNT(actor_id) >= 2
  SQL
end
...
def andrews_films_and_leads
  ...
  execute(<<-SQL)
    ...
    HAVING
      castings.ord = 1 AND 'Julie Andrews' IN (actors.name)
  SQL
end
...
def prolific_actors
  ...
  execute(<<-SQL)
    ...
    HAVING
      castings.ord = 1 AND COUNT(movies.title) >= 15
...
def films_by_cast_size
  ...
  execute(<<-SQL)
    ...
    JOIN
      actors ON castings.actor_id = actors.id
    GROUP BY movies.title, movies.yr
    HAVING
      movies.yr = 1978
...
def colleagues_of_garfunkel
  ...
  execute(<<-SQL)
    ...
    JOIN
      actors ON castings.actor_id = actors.id
    GROUP BY movies.title, actors.name
    HAVING
      movies.title IN (
        SELECT
          movies.title
